...
class BLOCK {
  public:
    ...
    // If the prefetch bit is set, indicates the cycle when the line was
    // filled via prefetching, if it has not been used yet.
    // However after the first use this no longer is indicative of anything
    // so we will set it to (uint64_t)(-1).
    uint64_t cycle_prefetched;
...
class PACKET {
  public:
    ...
    // We need to know when a read/write miss was merged with an outstanding
    // prefetch in order to report timeliness
    bool was_merged_with_prefetch_packet;
    uint64_t cycle_of_merge;

    // Priority
    // 1 - Highest priority, treated as a LOAD by the DRAM Controller
    // 2 - Lower priority, the controller may delay servicing this request for a
    // while. However, the controller will *not* NACK it.
    // 3 - The lowest priority. The controller may or may not service the
    // request. In the latter case a NACK is delivered to the LLC.
    uint8_t priority;
    ...
    PACKET() {
        ...
        was_merged_with_prefetch_packet = false;
        cycle_of_merge = 0;

        // By default, a packet has the highest priority
        priority = 1;
    };
};
