    ...
    for (match = 0; match < L2_ST_WAY; match++) {
        if (table[match].valid && (table[match].tag == tag)) { // Hit
            ...
            if (table[match].signature == 0) { // First hit in L2_ST
                ...
                // "the first hit in L2_ST" At this point, all we know about
                // this page is "the first accessed offset" We don't have any
                // delta information that can be a part of signature In other
                // words, the first offset does not update PT

                int sig_delta = curr_block - table[match].last_block;
                if (sig_delta < 0)
                    sig_delta =
                        64 + (curr_block - table[match].last_block) * (-1);
                ...
                if (warmup_complete[cpu])
                    L2_PF_DEBUG(printf(
                        "ST_hit_first cpu: %d cl_addr: %lx page: %lx "
                        "block: %d init_sig: %x delta: %d\n",
                        cpu, addr >> LOG2_BLOCK_SIZE, curr_page, curr_block,
                        table[match].signature, delta_buffer));
            } else {
                hit = 1;
                ...
                if (delta_buffer) {
                    ...
                    // L2 cache demand Now, the old signature will be associated
                    // with current delta
                ...
                if (warmup_complete[cpu])
                    L2_PF_DEBUG(printf(
                        "ST_hit cpu: %d cl_addr: %lx page: %lx block: %d "
                        "old_sig: %x delta: %d\n",
                        cpu, addr >> LOG2_BLOCK_SIZE, curr_page, curr_block,
    ...
    if (match == L2_ST_WAY) {
        for (match = 0; match < L2_ST_WAY; match++) {
            if (table[match].valid == 0) { // Invalid
                ...
                if (warmup_complete[cpu])
                    L2_PF_DEBUG(printf(
                        "ST_invalid cpu: %d cl_addr: %lx page: %lx block: %d\n",
                        cpu, addr >> LOG2_BLOCK_SIZE, curr_page, curr_block));
                break;
            }
        }
    ...
    if (match == L2_ST_WAY) { // Miss
        ...
        for (int i = 0; i < L2_GHR_TRACK; i++) {
            ...
            if ((spec_block == curr_block) &&
                ...
                if (warmup_complete[cpu])
                    L2_PF_DEBUG(printf(
                        "cpu: %d OOP_match  L2_GHR[%d]  signature: %x  "
                        "path_conf: "
                        "%d  last_block: %d  oop_delta: %d  spec_block: %d == "
                        "curr_block: %d  spec_sig: %x\n",
                        cpu, i, L2_GHR[cpu][i].signature,
                        L2_GHR[cpu][i].path_conf, L2_GHR[cpu][i].last_block,
                        L2_GHR[cpu][i].oop_delta, spec_block, curr_block,
                        spec_sig));
            } else {
                if (warmup_complete[cpu])
                    L2_PF_DEBUG(printf(
                        "cpu: %d OOP_unmatch  L2_GHR[%d]  signature: %x  "
        ...
        if (ghr_idx >= 0) {
            ...
            if (warmup_complete[cpu])
                L2_PF_DEBUG(
                    printf("cpu: %d spec_update  page: %x  sig: %3x  delta: "
...
int L2_ST_check(uint32_t cpu, uint64_t addr) {
    uint64_t curr_page = addr >> LOG2_PAGE_SIZE;
    int tag = curr_page & 0xFFFF, match = -1,
        L2_ST_idx = curr_page % L2_ST_PRIME;
    ...
    for (match = 0; match < L2_ST_WAY; match++) {
        if (table[match].valid && (table[match].tag == tag)) {
            if (warmup_complete[cpu])
                L2_PF_DEBUG(
                    printf("ST_check found cpu: %d cl_addr: %lx page: %lx "
...
void L2_PT_update(uint32_t cpu, int signature, int delta) {
    ...
    if (table[0].c_sig == (CSIG_MAX)) {
        ...
        if (warmup_complete[cpu])
            L2_PF_DEBUG(printf(
                "PT_sig: %4x cpu: %d c_sig saturated sig_total: %d => %d\n",
    ...
    for (match = 0; match < L2_PT_WAY; match++) {
        if (table[match].delta == delta) // Hit
        {
            ...
            if (warmup_complete[cpu])
                L2_PF_DEBUG(printf(
                    "PT_sig: %4x cpu: %d update_hit delta[%d]: %2d (%d / %d)\n",
            ...
            if (table[match].delta == 0) // Invalid
            {
                ...
                if (warmup_complete[cpu])
                    L2_PF_DEBUG(printf("PT_sig: %4x cpu: %d update_invalid "
                                       "delta[%d]: %2d (%d / %d)\n",
                                       signature, cpu, match,
                                       table[match].delta, table[match].c_delta,
                                       table[0].c_sig));
                L2_PT_invalid[cpu]++;
                L2_PT_access[cpu]++;
                break;
    ...
    if (match == L2_PT_WAY) // Miss
    {
        ...
        if (warmup_complete[cpu])
            L2_PF_DEBUG(printf(
                "PT_sig: %4x cpu: %d update_miss delta[%d]: %2d (%d / %d)\n",
