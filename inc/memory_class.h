...
class MEMORY {
  public:
    ...
    // empty queues
    PACKET_QUEUE WQ{"EMPTY", 1}, RQ{"EMPTY", 1}, PQ{"EMPTY", 1},
        MSHR{"EMPTY", 1};
    ...
    // We need to define the increase_priority function for all memory types,
    // even though only the DRAM exposes it. Same for checking availaility.
    // The opposite, for nack_request.
    virtual void increase_priority(PACKET *packet, uint8_t new_priority) = 0;
    virtual bool check_availability_for_priority_read(uint64_t address,
                                                      uint8_t priority) = 0;
    virtual void nack_request(PACKET *packet) = 0;
