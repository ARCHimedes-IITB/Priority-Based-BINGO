...
void MemoryWrite(VOID *addr, UINT32 index) {
    ...
    int already_found = 0;
    for (int i = 0; i < NUM_INSTR_DESTINATIONS; i++) {
        if (curr_instr.destination_memory[i] ==
...
VOID Instruction(INS ins, VOID *v) {
    ...
    for (UINT32 i = 0; i < readRegCount; i++) {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)RegRead, IARG_UINT32,
                       regNum, IARG_UINT32, i, IARG_END);
    }
    ...
    for (UINT32 i = 0; i < writeRegCount; i++) {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)RegWrite, IARG_UINT32,
                       regNum, IARG_UINT32, i, IARG_END);
    }
    ...
    for (UINT32 memOp = 0; memOp < memOperands; memOp++) {
        if (INS_MemoryOperandIsRead(ins, memOp)) {
            ...
            INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)MemoryRead,
                           ...
                           IARG_UINT32, read_size, IARG_END);
        }
...
int main(int argc, char *argv[]) {
    ...
    // cerr <<  "===============================================" << endl;
    // cerr <<  "This application is instrumented by the Champsim Trace
    // Generator"
    // << endl; cerr <<  "Trace saved in " << KnobOutputFile.Value() << endl;
    // cerr
    // <<  "===============================================" << endl;
