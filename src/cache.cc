...
void CACHE::handle_writeback() {
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) &&
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            // mark dirty
            // writeback hits do not set 'used'
            block[set][way].dirty = 1;
            ...
            if (cache_type == IS_L1D) { // RFO miss
                ...
                if (mshr_index == -2) {
                    ...
                    if ((mshr_index == -1) &&
                        ...
                        if (MSHR.entry[mshr_index].type == PREFETCH) {
                            // We still need to check priorities
                            // even if we are not at the LLC
                            if ((MSHR.entry[mshr_index].priority != 1))
                                lower_level->increase_priority(
                                    &MSHR.entry[mshr_index], 1);

                            // Writeback requests are not counted for
                            // timeliness related metrics.
                ...
                if (do_fill) {
                    ...
                    if (cache_type == IS_L1D)
                        l1d_prefetcher_cache_fill(
                            WQ.entry[index].full_addr, set, way, 0,
                            block[set][way].address << LOG2_BLOCK_SIZE,
            ...
            if (way >= 0) { // read hit
                ...
                if (block[set][way].prefetch) {
                    ...
                    if (!block[set][way].used) {
                        // Count timeliness
                        int64_t timeliness = (current_core_cycle[read_cpu] -
                                              block[set][way].cycle_prefetched);
                        if (report_timeliness_function) {
                            report_timeliness_function(
                                block[set][way].full_addr, timeliness,
                                current_core_cycle[read_cpu]);
                        }
                    }
                }
                ...
                if (mshr_index == -2) {
                    ...
                    if (cache_type == IS_LLC) {
                        ...
                        // Just keeping the old code around for now:
                        // if (lower_level->get_occupancy(
                        //         1, RQ.entry[index].address) ==
                        //     lower_level->get_size(1,
                        //     RQ.entry[index].address)) {

                        // Reads always get a priority 1
                        if (!lower_level->check_availability_for_priority_read(
                                RQ.entry[index].address, 1)) {
                            miss_handled = 0;
                        } else {
                    ...
                    if ((mshr_index == -1) &&
                        ...
                        // update request
                        if (MSHR.entry[mshr_index].type == PREFETCH) {
                            // If this prefetch had a priority that is not 1,
                            // we should ask the controller to increase its
                            // priority all the way to 1. Note that this should
                            // be done at all levels and not just the LLC: this
                            // is because, we may have prefetches from all they
                            // way to L1 be marked as priority 3: in this case
                            // there may be merged MSHR entries at levels 1 and
                            // 2 as well. We do this by calling the
                            // increase_priority function, which for caches does
                            // the relaying.
                            if ((MSHR.entry[mshr_index].priority != 1))
                                lower_level->increase_priority(
                                    &MSHR.entry[mshr_index], 1);
                            ...
                            // Timeliness: mark this MSHR entry
                            MSHR.entry[mshr_index]
                                .was_merged_with_prefetch_packet = true;
                            MSHR.entry[mshr_index].cycle_of_merge =
                                current_core_cycle[read_cpu];
...
void CACHE::handle_prefetch() {
    ...
    for (uint32_t i = 0; i < MAX_READ; i++) {
        ...
        // handle the oldest entry
        if ((PQ.entry[PQ.head].event_cycle <=
             current_core_cycle[prefetch_cpu]) &&
            (PQ.occupancy > 0)) {
            int index = PQ.head;
            ...
            if (way >= 0) { // prefetch hit
                ...
                if (all_warmup_complete >= NUM_CPUS &&
                    PQ.entry[index].pf_origin_level == FILL_L1) {
                    // Till where did the prefetch make it?
                    if (fill_level == FILL_L1)
                        l1d_prefetch_hit_at[0]++;
                    else if (fill_level == FILL_L2)
                        l1d_prefetch_hit_at[1]++;
                    else
                        l1d_prefetch_hit_at[2]++;
                }
                ...
                if (mshr_index == -2) {
                    ...
                    if (lower_level) {
                        if (cache_type == IS_LLC) {
                            // AB: Again, keeping old code around for now
                            // if (lower_level->get_occupancy(
                            ...
                            if (!lower_level
                                     ->check_availability_for_priority_read(
                                         PQ.entry[index].address,
                                         PQ.entry[index].priority))
                                miss_handled = 0;
                            else {
                    ...
                    if ((mshr_index == -1) &&
                        ...
                        // Also update priority if needed
                        if ((MSHR.entry[mshr_index].priority >
                             PQ.entry[index].priority))
                            lower_level->increase_priority(
                                &MSHR.entry[mshr_index],
                                PQ.entry[index].priority);
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet) {
    ...
    if (block[set][way].prefetch && (block[set][way].used == 0)) {
        pf_useless++;
        if (report_timeliness_function)
            report_timeliness_function(block[set][way].full_addr, 1000000,
                                       current_core_cycle[block[set][way].cpu]);
    }
    ...
    // Timeliness: we have two possible scenarios where timeliness needs to be
    // recored. First, if the packet being filled in is a prefetch, then we need
    // to record this cycle when we filled it in, so that we can record
    // timeliness the first time it is used.
    // On the other hand, we could have that the packet originally was a
    // prefetch and got merged with an actual access' miss; in such a case too
    // we should record the 'negative' timeliness.
    if (block[set][way].prefetch) {
        block[set][way].cycle_prefetched =
            current_core_cycle[block[set][way].cpu];
    } else if (packet->was_merged_with_prefetch_packet) {
        int64_t timeliness = int64_t(packet->cycle_of_merge) -
                             int64_t(current_core_cycle[block[set][way].cpu]);
        if (report_timeliness_function) {
            report_timeliness_function(block[set][way].full_addr, timeliness,
                                       current_core_cycle[packet->cpu]);
        }
    }
}
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr,
    ...
    if (PQ.occupancy < PQ.SIZE) {
        if ((base_addr >> LOG2_PAGE_SIZE) == (pf_addr >> LOG2_PAGE_SIZE)) {
            ...
            pf_packet.priority = 1;
            pf_stats[pf_packet.priority]++;
...
void CACHE::add_mshr(PACKET *packet) {
    ...
    for (index = 0; index < MSHR_SIZE; index++) {
        if (MSHR.entry[index].address == 0) {
            ...
            // We have not yet merged this packet with anyone
            MSHR.entry[index].was_merged_with_prefetch_packet = false;
            MSHR.entry[index].cycle_of_merge = 0;
...
/**
 * relay_increase_priority - Relay a request to increase the priority
 * of any packet with a given address.
 */
void CACHE::increase_priority(PACKET *packet, uint8_t new_priority) {
    // First increment the priority of any requests in the PQs.
    int index = PQ.check_queue(packet);
    if (index != -1 && PQ.entry[index].priority > new_priority)
        PQ.entry[index].priority = new_priority;

    // Do not return in either case; there could be an outstanding prefetch miss
    index = check_mshr(packet);
    if (index != -1 && MSHR.entry[index].priority > new_priority) {
        // We have such an outstanding miss
        MSHR.entry[index].priority = new_priority;
        // The same function name irrespective of whether the lower level is
        // DRAM
        lower_level->increase_priority(packet, new_priority);
    }
}

/**
 * nack_request - For prefetches that are marked with priority 3 (lowest), the
 * DRAM controller is free to refuse servicing them. In such a case, the
 * following function is called to free up the MSHR entry that the request
 * occupies.
 */
void CACHE::nack_request(PACKET *packet) {
    bool found = false;
    int xfill_level;
    int xcpu;
    for (uint32_t index = 0; index < MSHR_SIZE; index++) {
        if (MSHR.entry[index].address == packet->address) {
            found = true;
            xcpu = MSHR.entry[index].cpu;
            // Mark free
            MSHR.entry[index].address = 0;
            // At most one entry exists in the MSHR for any address
            xfill_level = MSHR.entry[index].fill_level;
            break;
        }
    }

    if (found && xfill_level < fill_level) {
        if (cache_type == IS_LLC)
            upper_level_dcache[xcpu]->nack_request(packet);
        else if (cache_type == IS_L2C) {
            upper_level_dcache[xcpu]->nack_request(packet);
            upper_level_icache[xcpu]->nack_request(packet);
        }
    }
}
