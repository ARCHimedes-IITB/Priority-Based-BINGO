...
uint32_t knob_low_bandwidth = 3200;
...
void print_roi_stats(uint32_t cpu, CACHE *cache) {
    ...
    cout << " AVERAGE MISS LATENCY: "
         ...
         << endl;
    // cout << " AVERAGE MISS LATENCY: " <<
    // (cache->total_miss_latency)/TOTAL_MISS
...
int main(int argc, char **argv) {
    ...
    while (1) {
        static struct option long_options[] = {
            ...
            {"low_bandwidth", required_argument, 0, 'b'},
            {"traces", no_argument, 0, 't'},
            {0, 0, 0, 0}};

        int option_index = 0;

        c = getopt_long_only(argc, argv, "wihcb", long_options, &option_index);
        ...
        case 'c':
            ...
            break;
        case 'b':
            knob_low_bandwidth = atol(optarg);
            break;
        case 't':
    ...
    // DRAM_MTPS = DRAM_IO_FREQ * knob_low_bandwidth / 10;
    DRAM_MTPS = knob_low_bandwidth;
    ...
    for (int i = 0; i < argc; i++) {
        if (found_traces) {
            ...
            std::string last_dot =
                full_name.substr(full_name.find_last_of("."));
            ...
            if (count_traces > NUM_CPUS) {
                printf("\n*** Too many traces for the configured number of "
                       "cores ***\n\n");
                assert(0);
            }
    ...
    for (int i = 0; i < NUM_CPUS; i++) {
        ...
        // ooo_cpu[i].DTLB.MAX_READ = (2 > MAX_READ_PER_CYCLE) ?
        ...
        ooo_cpu[i].L1I.l1i_prefetcher_cache_fill =
    ...
    while (run_simulation) {
        ...
        for (int i = 0; i < NUM_CPUS; i++) {
            ...
            if (all_warmup_complete ==
                NUM_CPUS) { // this part is called only once
    ...
    uncore.LLC.llc_prefetcher_final_stats();
    int total_l1d_prefs = l1d_prefetch_hit_at[0] + l1d_prefetch_hit_at[1] +
                          l1d_prefetch_hit_at[2] + l1d_prefetch_hit_at[3];
    cout << "Of all L1D Prefetches, percentage that made it to:" << endl;
    cout << "L1D: " << (100.0 * l1d_prefetch_hit_at[0] / total_l1d_prefs)
         << endl;
    cout << "L2C: " << (100.0 * l1d_prefetch_hit_at[1] / total_l1d_prefs)
         << endl;
    cout << "LLC: " << (100.0 * l1d_prefetch_hit_at[2] / total_l1d_prefs)
         << endl;
    cout << "DRAM: " << (100.0 * l1d_prefetch_hit_at[3] / total_l1d_prefs)
         << endl;
    cout << "Percentage of reads to DRAM that are L1D-initiated prefetches: "
         << (100.0 * l1d_prefetch_hit_at[3] / dram_reads) << endl;

    cout << "At L1D number of prefetches with priority " << endl;
    for (int c = 0; c < NUM_CPUS; c++) {
        for (int i = 1; i < 4; i++) {
            cout << i << " : " << ooo_cpu[c].L1D.pf_stats[i] << endl;
        }
    }
