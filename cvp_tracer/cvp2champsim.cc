...
struct trace {
    ...
    // read a single record from the trace file, return true on success, false
    // on EOF
    ...
    bool read(FILE *f) {
        ...
        case uncondIndirectBranchInstClass:
            ...
            if (taken) {
                ...
                // this had better not be an unconditional branch (frickin ARM
                // with its predicated branches)
...
FILE *open_trace_file(void) {
    ...
    if (!strcmp(tracefilename, "-")) {
        ...
        if (s[0] == 0xfd && s[1] == '7' && s[2] == 'z' && s[3] == 'X' &&
            ...
            // start up an xz decompression and open a pipe to our standard
            // input
        ...
        // check for the magic number for GZIP compression
            ...
            // use Unix cat to open and read this file. we could just fopen the
            // file but then we're pclosing it later so that could get weird
...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        // if this is a branch then do more stuff; we don't care about
        // non-branches
        ...
        if (is_branch(t.type)) {
            ...
            if (t.type == condBranchInstClass) {
                ...
                // this is some other kind of branch. it should have a non-zero
                // target

                assert(t.target);

                // on ARM, calls link the return address in register X30. let's
                // see if this instruction is doing that; if so, it's a call or
                ...
                // on ARM, returns are an indirect jump to X30. let's see if
                // we're doing this
            ...
            memset(ct.destination_registers, 0,
                   sizeof(ct.destination_registers));
            ...
            case OPTYPE_JMP_DIRECT_COND:
                ...
                ct.destination_registers[0] = REG_IP;
                // turns out pin records conditional direct branches as also
                // reading IP. whatever.
            ...
            case OPTYPE_CALL_INDIRECT_UNCOND:
                ...
                // reads something else, reads IP, reads SP, writes SP, writes
                // IP
                ct.destination_registers[0] = REG_IP;
